...
# When primary, queue, cache, and cable all use the same DATABASE_URL (e.g. Render + Supabase),
# `db:migrate` only applied db/migrate to that database — Solid* tables lived only in
# queue_schema.rb / cache_schema.rb / cable_schema.rb and were never created in production.
class CreateSolidQueueCacheCableTables < ActiveRecord::Migration[8.0]
  def up
    create_solid_queue_tables
    create_solid_cache_table
    create_solid_cable_table
